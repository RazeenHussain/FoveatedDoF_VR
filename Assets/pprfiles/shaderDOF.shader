...
//			_ViewDepth = depth0;
//			_ViewDepth = tex2D(_MainTex, float2(_centreH,_centreV));
//			if (((depth > (_ViewDepth - tolDepth)) && (depth < (_ViewDepth + tolDepth))) || (len <= _Radius))
			...
			else
			{
				...
				for (float indexY = 0; indexY < SAMPLES; indexY++)
				{
					float offsetY = (indexY / (SAMPLES - 1) - 0.5) * bSize;
					for (float indexX = 0; indexX < SAMPLES; indexX++)
					{ 
						float offsetX = (indexX / (SAMPLES - 1) - 0.5) * bSize;
						...
						else
						{
							...
							float gaussian = (1 / sqrt(2 * PI * StDevSq)) * pow(E, -(((offsetX*offsetX)+ (offsetY*offsetY)) / (2 * StDevSq)));
							sum += gaussian;
